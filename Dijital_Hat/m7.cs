...
namespace Dijital_Hat
{
    public partial class m7 : Form
    {
        ...
        mo m = new mo();
        int index = 0;
        private void m7_Load(object sender, EventArgs e)
        {
            m_kisitla();
            timer1.Enabled = true;
            ...
            komut1.CommandText = ("Select * From deprem_8");
            ...
            while (oku1.Read())
            {
                ...
                listBox_y_g.Items.Add(Math.Round(D1x[index], 7));

                listBox_z_g.Items.Add(Math.Round(D1x[index], 7));
                index++;
            ...
            textBox2.Text =m. ambrayses(D1x.Max(), mc7).ToString();
            textBox3.Text =m. ambrayses(D1y.Max(), mc7).ToString();
            textBox4.Text =m. ambrayses(D1z.Max(), mc7).ToString();
        }
